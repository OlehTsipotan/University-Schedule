...
public class UserControllerTest {
	...
	public void register_whenValidUser_thenRedirectLogin() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/user/register").with(csrf())
						.flashAttr("userRegisterDTO", userRegisterDTO)).andExpect(status().is3xxRedirection())
				.andExpect(view().name("redirect:/user/login")).andExpect(flash().attributeExists("registerSuccess"));
	...
	public void register_whenInvalidUser_thenProcessForm() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/user/register").with(csrf())
						.flashAttr("userRegisterDTO", userRegisterDTO)).andExpect(status().is2xxSuccessful())
				.andExpect(view().name("login/userRegister")).andExpect(model().attributeExists("roleDTOList"));
