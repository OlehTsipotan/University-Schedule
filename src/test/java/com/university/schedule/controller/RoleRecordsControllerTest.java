...
public class RoleRecordsControllerTest {
    ...
    public void update_whenNotValidRoleByEmptyField_thenProcessForm() throws Exception {
        ...
        // Act and Assert
        mockMvc.perform(
                MockMvcRequestBuilders.post("/roles/update/{id}", roleId).with(csrf()).flashAttr("roleDTO", invalidRoleDTO))
            .andExpect(status().is2xxSuccessful()).andExpect(model().attributeExists("entity"))
            .andExpect(model().attribute("entity", invalidRoleDTO)).andExpect(view().name("rolesUpdateForm"));
