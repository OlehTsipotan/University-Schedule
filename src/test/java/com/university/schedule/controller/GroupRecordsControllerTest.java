...
public class GroupRecordsControllerTest {
    ...
    public void getAll_whenNoArgs_happyPath() throws Exception {
        ...
        // Act and Assert
        mockMvc.perform(MockMvcRequestBuilders.get("/groups")).andExpect(status().is2xxSuccessful())
            .andExpect(view().name("groups")).andExpect(
                model().attributeExists("entities", "currentLimit", "currentOffset", "sortField", "sortDirection",
                    "reverseSortDirection")).andExpect(model().attribute("entities", groupDTOList));
    ...
    public void getAll_whenLimitAndOffsetArgs_happyPath(int limit, int offset) throws Exception {
        ...
        // Act and Assert
        mockMvc.perform(MockMvcRequestBuilders.get("/groups").param("limit", String.valueOf(limit))
                .param("offset", String.valueOf(offset))).andExpect(status().is2xxSuccessful())
            .andExpect(view().name("groups")).andExpect(
                model().attributeExists("entities", "currentLimit", "currentOffset", "sortField", "sortDirection",
                    "reverseSortDirection")).andExpect(model().attribute("entities", groupDTOList));
    ...
    public void delete_whenGroupServiceThrowsDeletionFailedException_thenProcessError() throws Exception {
        ...
        // Act and Assert
        mockMvc.perform(MockMvcRequestBuilders.get("/groups/delete/{id}", groupId)).andExpect(status().isOk())
    ...
    public void update_whenValidGroup_thenRedirectSuccess() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.post("/groups/update/{id}", groupId).with(csrf())
                .flashAttr("groupDTO", validGroupDTO)).andExpect(status().is3xxRedirection())
    ...
    public void update_whenNotValidGroupByEmptyField_thenProcessForm() throws Exception {
        ...
        // Act and Assert
        mockMvc.perform(MockMvcRequestBuilders.post("/groups/update/{id}", groupId).with(csrf())
                .flashAttr("groupDTO", invalidGroupDTO)).andExpect(status().is2xxSuccessful())
            .andExpect(model().attributeExists("entity")).andExpect(model().attribute("entity", invalidGroupDTO))
    ...
    public void update_whenGroupServiceThrowValidationException_thenProcessForm() throws Exception {
        ...
        // Act and Assert
        mockMvc.perform(
                MockMvcRequestBuilders.post("/groups/update/{id}", groupId).with(csrf()).flashAttr("groupDTO", groupDTO))
    ...
    public void update_whenGroupServiceThrowServiceException_processError() throws Exception {
        ...
        mockMvc.perform(
                MockMvcRequestBuilders.post("/groups/update/{id}", groupId).with(csrf()).flashAttr("groupDTO", groupDTO))
            .andExpect(status().is2xxSuccessful()).andExpect(model().attributeExists("exceptionMessage"))
            .andExpect(view().name("error")).andExpect(model().attribute("exceptionMessage", exceptionMessage));
    ...
    public void getInsertForm_happyPath() throws Exception {
        ...
        // Act and Assert
        mockMvc.perform(MockMvcRequestBuilders.get("/groups/insert")).andExpect(status().is2xxSuccessful())
    ...
    public void insert_whenValidGroup_thenRedirectSuccess() throws Exception {
        ...
        // Act and Assert
        mockMvc.perform(MockMvcRequestBuilders.post("/groups/insert").with(csrf()).flashAttr("groupDTO", validGroupDTO))
            .andExpect(status().is3xxRedirection()).andExpect(view().name("redirect:/groups"))
    ...
    public void insert_whenNotValidGroupByEmptyField_thenProcessForm() throws Exception {
        ...
        // Act and Assert
        mockMvc.perform(
                MockMvcRequestBuilders.post("/groups/insert").with(csrf()).flashAttr("groupDTO", invalidGroupDTO))
